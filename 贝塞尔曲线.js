...
ui.layout(
    <vertical>
        ...
        <horizontal h="auto">
            <button id="but_B" layout_weight="1" h="auto" text="最后添加一点"/>
            <button id="but_C" layout_weight="1" h="auto" text="去掉最后一点"/>
        </horizontal>
    </vertical>
);
...
ui.canvas.on("draw", function(canvas) {
    ...
    paint.setStyle(Paint.Style.FILL);
    try {
        Curve(canvas, pointsAry, D, paint);
        //递归绘制
    } catch (e) {
        toastLog(e);
    };
...
function Curve(canvas, pointsAry, D, paint, C) {
    C = C || 0;
    var Ary = new Array;
    ...
    if (Ary.length) {
        arguments.callee(canvas, Ary, D, paint, C + 1);
    } else {
        let length = pointsAry.length;
        if (length) {
